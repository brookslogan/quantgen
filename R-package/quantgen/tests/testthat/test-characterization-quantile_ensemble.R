...
#' A custom version of \code{\link[testthat:expect_snapshot]{testthat::expect_snapshot}} that saves an object to an RDS file in the \code{tests/testthat/_custom_snaps} directory
#'
...
test_that("quantile_ensemble's A matrix (for Rgplk) encodes identical entries to previous version", {
  withr::with_rng_version("3.6.0", withr::with_seed(222017L, {
    ...
    test.object.names = character(0L)
    test.object.i = 0L
    for (tau_groups_i in seq_along(tau_groups_choices)) {
      ...
      for (partial.config in partial.configs) {
        ...
        if (!(length(unique(tau_groups)) != 1L && partial.config[["intercept"]] && partial.config[["noncross"]])) {
          test.object.i <- test.object.i + 1L
          test.object.names[[test.object.i]] = do.call(sprintf, c(list("transformed-glpk-inputs-%d%d%d%d%d%s", tau_groups_i), partial.config))
          quantile_ensemble(
    ...
    ##   Make sure that the test object names don't contain duplicates (else testing bug):
    stopifnot(anyDuplicated(test.object.names) == 0L)
    ##   Test that we have the same number of test objects and test object names:
    testthat::expect_identical(length(reformatted.arg.lists), length(test.object.names))
    ##   Test the test objects for changes:
    for (i in seq_along(reformatted.arg.lists)) {
      ## `expect_snapshot_value` has issues with readable `style`s and an RDS seems
      ## preferable to the "serialize" style.
      expect_identical_custom_rds_snapshot(reformatted.arg.lists[[i]], test.object.names[[i]])
    }
  }))
})

## TODO check that different matrix types actually act the same within solver.
